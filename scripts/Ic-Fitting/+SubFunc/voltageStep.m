function voltageStep(sweepMode, newFolderPath)
    % Set default sweep mode if not provided
    if nargin < 2
        sweepMode = 'NegPosSweep';
    end

    % Get a list of all txt files in the specified folder
    files = dir(fullfile(newFolderPath, '*.txt'));

    % Loop through each file and analyze the V-I characteristic curve
    for file = files'
        file_path = fullfile(newFolderPath, file.name);
        data = load(file_path);
        ...
        xlabel('Current (A)');
        ylabel('Voltage (V)');
        title(['Voltage-Current Characteristic Curve with Step Regions Highlighted - ', file.name]);
        grid on;
        hold on;
        ...
        for i = 1:size(step_regions, 1)
            fprintf("Step region %d in file %s: Current range from %.6f A to %.6f A\n", i, file.name, current(step_regions(i, 1)), current(step_regions(i, 2)));
        end
    end
end
